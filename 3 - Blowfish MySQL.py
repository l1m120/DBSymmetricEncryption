...
def generate_strong_key():
    """Generates a cryptographically secure random key for Blowfish (448 bits)."""
...
def encrypt_data(data):
    """Encrypt data using Blowfish (448 bits) and Base64 encode."""
...
def decrypt_data(encoded_data):
    """Decrypt Base64 encoded data using Blowfish (448 bits)."""
    decoded_data = base64.b64decode(encoded_data.encode('utf-8'))
    cipher = Blowfish.new(key_bytes, Blowfish.MODE_CBC, iv)
    decrypted_padded = unpad(cipher.decrypt(decoded_data), block_size)
...
import secrets
key = secrets.token_bytes(56)  # 448-bit (56-byte) key for Blowfish
key_hex = key.hex()
os.environ["ENCRYPTION_KEY"] = key_hex
print(f"Generated Key (Hex): {key_hex}")
